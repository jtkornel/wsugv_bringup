...
Bringup package for Waveshare UGV
...
If you are familiar with ROS 2, here are the quick-and-dirty build instructions.
  ...
  git clone git@github.com:wsugv_bringup/wsugv_bringup.git src/wsugv_bringup
  vcs import src --input src/wsugv_bringup/wsugv_bringup.humble.repos
  vcs import src --input src/wsugv_bringup/wsugv_bringup.humble.upstream.repos
...
1. Start robot's hardware and load controllers (default configuration starts mock hardware)
   ```
   ros2 launch wsugv_bringup wsugv.launch.xml
   ```
...
1. If you want to test hardware with `ForwardCommandController` first load and configure it. Controller types are e.g., "position", "velocity", and depend on configured names in the [`config/wsugv_controllers.yaml`](config/wsugv_controllers.yaml):
   ```
...
3. Send command to the controller, either:
   ...
   ```
   ros2 launch wsugv_bringup test_forward_position_controller.launch.py
   ```
